...
function lazyObjectReadable(fn) {
    return new lazystream_1.default.Readable(fn, { objectMode: true });
}
...
function buildComponent({ projectConfig, component, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    if (!entryPoint)
        return;
    return lazyObjectReadable(() => compile_1.default({
        component,
        output,
        onDiagnostic,
        input: entryPoint,
...
function buildDeviceResources(projectConfig, { resourceFilterTag }, onDiagnostic = diagnostics_1.logDiagnosticToConsole) {
    const convertImageToTXIOptions = {};
    if (sdkVersion_1.default().major >= 2) {
        convertImageToTXIOptions.rgbaOutputFormat = convertImageToTXI_1.TXIOutputFormat.RGBA6666;
    }
    return new pumpify_1.default.obj(filterResourceTag_1.default(resourceFilterTag), validateIcon_1.default({ projectConfig, onDiagnostic }), convertImageToTXI_1.default(convertImageToTXIOptions), vinylAssertFiles_1.default([resources.svgMain, resources.svgWidgets]));
}
...
function buildDeviceComponents({ projectConfig, buildId, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    if (sdkVersion_1.default().major < 3) {
        deviceJSPipeline.push(gulpMagicString_1.default(errataWorkarounds_1.errataPrimaryExpressionInSwitch));
    }
    ...
    return multistream_1.default.obj([
        ...
        ...projectConfig.buildTargets.map((family) => lazyObjectReadable(() => {
            const { platform, displayName } = buildTargets_1.default[family];
            onDiagnostic({
            ...
            return new pumpify_1.default.obj(merge_stream_1.default(new pumpify_1.default.obj(processedJS.newReadableSide({ objectMode: true }), sourceMap.collector(componentTargets_1.ComponentType.DEVICE, family)), new pumpify_1.default.obj(vinyl_fs_1.default.src('./resources/**', { base: '.' }), buildDeviceResources(projectConfig, buildTargets_1.default[family], onDiagnostic))), componentManifest_1.makeDeviceManifest({ projectConfig, buildId }), zip_1.default(`device-${family}.zip`, { compress: sdkVersion_1.default().major >= 3 }), gulpSetProperty_1.default({
                componentBundle: {
                    family,
                    platform,
                    type: 'device',
